"""Resolve default config and package paths (taxipp, france-data, CI, tests)."""
...
openfisca_data_manager_location = Path(__file__).resolve().parent.parent.parent
...
# Minimal generated config directory used when no user config is found.
config_files_templates_directory = (
    openfisca_data_manager_location / "openfisca_data_manager" / "config_files_templates"
)
test_config_files_directory = Path(tempfile.gettempdir()) / "openfisca-data-manager-config"
test_config_files_directory.mkdir(parents=True, exist_ok=True)

with (config_files_templates_directory / "config_template.ini").open() as file:
    config_ini = file.read()

config_ini = config_ini.format(location=openfisca_data_manager_location)
try:
    with (test_config_files_directory / "config.ini").open("w+") as file:
        file.write(config_ini)
except PermissionError:
    log.debug(
        "config.ini can't be written in the test config files directory %s",
...
if (is_in_ci and default_config_files_directory is None) or ("pytest" in sys.modules):
    if "CI_RUNNER_TAGS" in os.environ:
        private_run_with_data = "data-in" in os.environ["CI_RUNNER_TAGS"]
...
if default_config_files_directory is None:
    default_config_files_directory = BaseDirectory.save_config_path("openfisca-survey-manager")
    log.debug("Using default_config_files_directory = %s", default_config_files_directory)
